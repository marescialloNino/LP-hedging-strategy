...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
• Reads price matrix + closed/open positions from python/krystal_pnl/pnl_data  
• Adds closed‑PnL and open‑PnL **but outputs only pools that are still open**  
• Final table is written to lp-data/krystal_pnl_by_pool.csv
"""
...
DETAIL_CSV          = INT_DATA_DIR / "closed_positions_pnl.csv"            # optional detail
...
def compute_L(x0: float, y0: float, p_min: float, p_max: float) -> float:
    ...
    A, B, C = alpha * beta - 1.0, x0 * alpha + y0 * beta, x0 * y0
    disc = B * B - 4 * A * C
    if disc < 0:
        raise ValueError("No real L")
    r1 = (-B + math.sqrt(disc)) / (2 * A)
    r2 = (-B - math.sqrt(disc)) / (2 * A)
    L = max(r1, r2)
    if L <= 0:
        raise ValueError("No positive L")
    return L

def solve_v3_withdrawals(W, Pa, Pb, p, L, p_min, p_max):
    s_eta, inv_s_beta = math.sqrt(p_min), 1.0 / math.sqrt(p_max)
...
for _, r in closed_raw.iterrows():
    ...
    if None in (pA0, pB0, pA1, pB1):
        continue
    if any(x <= 0 for x in [pA0, pB0, pA1, pB1, r["totalDepositValue"], r["totalWithdrawValue"]]):
        continue
    ...
    if P_close <= r["minPrice"]:
        QA1, QB1 = W / pA1, 0.0
    elif P_close >= r["maxPrice"]:
        QA1, QB1 = 0.0, W / pB1
    else:
        QA1, QB1 = solve_v3_withdrawals(W, pA1, pB1, P_close, L, r["minPrice"], r["maxPrice"])

    val0 = QA0 * P_open + QB0
    val1 = QA1 * P_close + QB1
    pnl_tokenB = val1 - val0
    pnl_usd = r["totalWithdrawValue"] - dep
...
closed_agg = (
    closed_detail.groupby(["chainName", "poolAddress"])
    .agg(
...
for _, r in open_raw.iterrows():
    pA1, pB1 = r["tokenA_price"], r["tokenB_price"]
    if None in (pA1, pB1) or any(x <= 0 for x in [pA1, pB1]):
        continue
    QA0, QB0, QA1, QB1 = (
        r["tokenA_provided"],
        r["tokenB_provided"],
    ...
    if any(x < 0 for x in [QA0, QB0, QA1, QB1]):
        continue
    ...
    val0 = QA0 * P_open + QB0
    val1 = QA1 * P_close + QB1 + feeA * P_close + feeB
    pnl_tokenB = val1 - val0
...
# ╭────────── 5) MERGE: KEEP ONLY POOLS WITH OPEN POSITIONS ─────╮
# left‑join open_agg with closed_agg, fill NaN with 0, then sum
final_df = open_agg.merge(
    closed_agg[["chainName", "poolAddress", "pnl_usd", "pnl_tokenB"]],
    on=["chainName", "poolAddress"],
    how="left",
    suffixes=("_open", "_closed"),
)
final_df[["pnl_usd_closed", "pnl_tokenB_closed"]] = final_df[
    ["pnl_usd_closed", "pnl_tokenB_closed"]
].fillna(0)

final_df["pnl_usd_total"] = final_df["pnl_usd_open"] + final_df["pnl_usd_closed"]
final_df["pnl_tokenB_total"] = final_df["pnl_tokenB_open"] + final_df["pnl_tokenB_closed"]
...
].rename(columns={"pnl_usd_total": "pnl_usd", "pnl_tokenB_total": "pnl_tokenB"})

final_df.to_csv(FINAL_PNL_CSV, index=False)

print(f"🗂  Final PnL (only pools with open positions) → {FINAL_PNL_CSV.relative_to(ROOT_DIR)}")
print(f"Closed pools processed: {len(closed_detail)}  |  Open pools processed: {len(open_rows)}")
